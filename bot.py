...
async def on_message(message):
    # Check if the message landed in your personal mirrored channel
    if message.channel.id == CHANNEL_ID:
        
        # Verify the message originally came from the source notifier channel ID
        is_from_source = False
        if message.reference and message.reference.channel_id == SOURCE_CHANNEL_ID:
            is_from_source = True
        # Back-up check: compatibility for Webhook cross-posts that use message flags
        elif message.flags.value & 256: # FLAG_SOURCED_FROM_NOTIFICATION_CHANNEL
            is_from_source = True
            
        # If it passes validation, scrape the text and save the data!
        if is_from_source:
            text_to_scan = ""

            # 1. Gather all plain text content
            if message.content:
                text_to_scan += message.content.title()

            # 2. Gather all embed content blocks
            if message.embeds:
            ...
            # 3. Match text blobs to update timestamps
            found_items = []
